...
# MAGIC ## Summary
# MAGIC
# MAGIC You've successfully demonstrated:
# MAGIC
# MAGIC 1. ✅ **RBAC Limitations**: Manual, doesn't scale, no fine-grained control
# MAGIC 2. ✅ **ABAC Setup**: Tags → UDFs → Policies → Automatic enforcement
# MAGIC 3. ✅ **Column Masking**: Automatic PII protection (SSN, email, credit card, salary)
# MAGIC 4. ✅ **Row Filtering**: Regional data isolation for compliance
# MAGIC 5. ✅ **Scalability**: One policy protects entire catalog
# MAGIC
# MAGIC ## Next Steps
# MAGIC
# MAGIC - **Production Implementation**: Use governed tags (not free-form)
# MAGIC - **User Groups**: Create proper user groups (US_Analysts, EU_Analysts, etc.)
# MAGIC - **Extended Policies**: Add policies for more sensitive attributes
# MAGIC - **Monitoring**: Set up audit logging and compliance reports
# MAGIC - **Documentation**: Document tag taxonomy and policy definitions
# MAGIC
# MAGIC ## Resources
# MAGIC
# MAGIC - [Unity Catalog ABAC Documentation](https://docs.databricks.com/security/abac.html)
# MAGIC - [Governed Tags Guide](https://docs.databricks.com/data-governance/unity-catalog/tags.html)
# MAGIC - [Best Practices for Data Governance](https://docs.databricks.com/data-governance/index.html)
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ### 📧 Questions?
# MAGIC
# MAGIC Contact your Databricks account team for:
# MAGIC - Governance architecture reviews
# MAGIC - ABAC policy design workshops
# MAGIC - Compliance requirements mapping
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC **Thank you for attending this demo!** 🚀

# COMMAND ----------

# DBTITLE 1,How to Run This Demo
# MAGIC %md
# MAGIC # 🚀 How to Run This Demo
# MAGIC
# MAGIC ## Quick Start Instructions
# MAGIC
# MAGIC ### Step-by-Step Execution:
# MAGIC
# MAGIC 1. **Update Configuration** (Cell 3)
# MAGIC    - Modify `catalog_name` and `schema_name` if desired
# MAGIC    - Current setup uses meaningful retail names:
# MAGIC      - Catalog: `retail_corp`
# MAGIC      - Schema: `customer_analytics`
# MAGIC      - Tables: `customers`, `orders`, `employees`
# MAGIC
# MAGIC 2. **Run Setup Cells** (Cells 5-12)
# MAGIC    - Creates catalog, schema, and sample retail tables
# MAGIC    - Applies governed tags to sensitive columns
# MAGIC    - Takes ~3 minutes
# MAGIC
# MAGIC 3. **Review RBAC Limitations** (Cells 13-14)
# MAGIC    - Discussion section (no execution needed)
# MAGIC
# MAGIC 4. **Create ABAC Components** (Cells 15-27)
# MAGIC    - Creates masking UDFs (SSN, email, credit card, salary)
# MAGIC    - Creates row filter UDFs (regional isolation)
# MAGIC    - Creates ABAC policies at catalog level
# MAGIC    - Takes ~5 minutes
# MAGIC
# MAGIC 5. **Run Demonstrations** (Cells 28-36)
# MAGIC    - Shows column masking in action
# MAGIC    - Shows row-level filtering
# MAGIC    - Takes ~5 minutes
# MAGIC
# MAGIC 6. **Cleanup** (Cells 39-42)
# MAGIC    - Removes all demo resources
# MAGIC    - Makes demo fully rerunnable
# MAGIC    - Takes ~1 minute
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ### ⏱️ Timing Guide (Total: 30 minutes)
# MAGIC
# MAGIC | Part | Time | Description |
# MAGIC |------|------|-------------|
# MAGIC | Part 1: Setup | 5 min | Create catalog, schema, tables, tags |
# MAGIC | Part 2: RBAC Discussion | 3 min | Explain traditional limitations |
# MAGIC | Part 3: ABAC Setup | 10 min | Create UDFs and policies |
# MAGIC | Part 4: Demonstrations | 10 min | Show masking & filtering live |
# MAGIC | Part 5: Cleanup | 2 min | Remove all resources |
# MAGIC
# MAGIC ---
# MAGIC
...
# MAGIC - `USE CATALOG` on system catalog (for system tables)
# MAGIC - `CREATE CATALOG` privilege
# MAGIC - `CREATE FUNCTION` privilege
# MAGIC - Policy management permissions (may require account admin)
# MAGIC
# MAGIC **Runtime Requirements:**
# MAGIC - Databricks Runtime 16.4+ or Serverless Compute
# MAGIC - Unity Catalog enabled workspace
# MAGIC - Fine-grained access control enabled (for dedicated clusters)
# MAGIC
# MAGIC **Rerunnable Design:**
# MAGIC - Complete cleanup at end allows full re-execution
# MAGIC - All objects created with `IF NOT EXISTS` or `OR REPLACE`
# MAGIC - No manual cleanup needed between runs
# MAGIC
# MAGIC **Production Considerations:**
# MAGIC - This demo uses **free-form tags** for simplicity
# MAGIC - In production, use **governed tags** with enforced values
# MAGIC - Create proper user groups (US_Analysts, EU_Analysts, HR_Admins, etc.)
# MAGIC - Test policies with different user personas
# MAGIC - Set up audit logging for compliance tracking
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ### 📊 Expected Outcomes
# MAGIC
# MAGIC After running this demo, you'll have demonstrated:
# MAGIC
# MAGIC ✅ **Column Masking**: SSN → XXX-XX-6789, Email → a\*\*\*e@email.com  
# MAGIC ✅ **Financial Protection**: Credit card masking, salary ranges  
# MAGIC ✅ **Row Filtering**: Regional data isolation (US/EU/APAC)  
# MAGIC ✅ **Scalability**: One catalog-level policy protects all tables  
# MAGIC ✅ **Automatic Inheritance**: New tables inherit governance automatically  
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ### 🎯 Presentation Tips
# MAGIC
# MAGIC 1. **Start with the problem**: Show RBAC limitations (Part 2) before ABAC solution
# MAGIC 2. **Use the diagrams**: Point out the ASCII visualizations in markdown cells
# MAGIC 3. **Live queries**: Run the demonstration queries and show masked vs unmasked data
# MAGIC 4. **Emphasize automation**: Highlight that policies apply without code changes
# MAGIC 5. **Real-world context**: Reference GDPR, HIPAA, PCI-DSS compliance needs
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC **Ready? Let's begin! 👇 Start with Cell 3 (Configuration)**

# COMMAND ----------

# DBTITLE 1,✅ Demo Status: READY
# MAGIC %md
# MAGIC # ✅ Demo Status: FULLY OPERATIONAL
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ## 🎉 Your ABAC Demo is Ready!
# MAGIC
# MAGIC ### Current Status:
# MAGIC
# MAGIC | Component | Status | Details |
# MAGIC |-----------|--------|----------|
# MAGIC | **Catalog & Schema** | ✅ Created | `retail_corp.customer_analytics` |
# MAGIC | **Demo Tables** | ✅ Populated | 8 customers, 10 orders, 8 employees |
# MAGIC | **Governed Tags** | ✅ Applied | `pii`, `sensitivity`, `geo_region`, `department` |
# MAGIC | **Masking UDFs** | ✅ Created | SSN, Email, Credit Card, Salary |
# MAGIC | **Row Filter UDFs** | ✅ Created | Regional isolation (US/EU/APAC) |
# MAGIC | **Column Mask Policies** | ✅ Active | 4 policies on catalog |
# MAGIC | **Xref Table** | ✅ Created | User-group mapping with 4 groups |
# MAGIC | **Your Access Level** | ✅ Policy Owner | Sees all data unmasked |
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ## 🚀 How to Demo (3 Options)
# MAGIC
# MAGIC ### Option 1: Single-User Demo (Recommended for Solo Testing)
# MAGIC
# MAGIC Test all 4 access levels yourself:
# MAGIC
# MAGIC 1. **Current State**: Run Cell 34 → See full unmasked data (policy_owner)
# MAGIC 2. **Switch to data_analysts**:
# MAGIC    ```sql
# MAGIC    UPDATE retail_corp.customer_analytics.user_group_mapping 
# MAGIC    SET group_name = 'data_analysts' 
# MAGIC    WHERE user_email = current_user();
# MAGIC    ```
# MAGIC 3. **Restart Python kernel** → Re-run Cell 34 → See masked PII
# MAGIC 4. **Switch to finance_team** → Restart → See financial data unmasked
# MAGIC 5. **Switch to us_regional_analysts** → Restart → See only US customers
# MAGIC
# MAGIC ### Option 2: Multi-User Demo (Best for Live Presentations)
# MAGIC
# MAGIC 1. Add colleagues to the demo groups (Cell 27 creates the groups)
# MAGIC 2. Have them open this notebook and run Cell 34
# MAGIC 3. Compare results side-by-side → Same query, different data!
# MAGIC
# MAGIC ### Option 3: Quick Validation (30 seconds)
# MAGIC
# MAGIC 1. Run Cell 35 → Verify all components exist
# MAGIC 2. Run Cell 34 → See your current access level
# MAGIC 3. Done! You're seeing ABAC in action
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ## 📊 What Each Group Sees
# MAGIC
# MAGIC | Group | SSN | Email | Credit Card | Salary | Customers Visible |
# MAGIC |-------|-----|-------|-------------|--------|-------------------|
# MAGIC | **policy_owner** | `123-45-6789` | `alice@email.com` | `1234` | `$85,000.00` | All 8 (all regions) |
# MAGIC | **data_analysts** | `XXX-XX-6789` | `a***e@email.com` | `****1234` | `High` | All 8 (all regions) |
# MAGIC | **finance_team** | `123-45-6789` | `a***e@email.com` | `1234` | `$85,000.00` | All 8 (all regions) |
# MAGIC | **us_regional_analysts** | `XXX-XX-6789` | `a***e@email.com` | `****1234` | `High` | **Only 4 (US only)** |
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ## 🎯 Key Demo Messages
# MAGIC
# MAGIC 1. **Same SQL, Different Results** → ABAC dynamically masks based on identity
# MAGIC 2. **No Code Changes** → Policies apply automatically to all queries
# MAGIC 3. **Catalog-Level Governance** → One policy protects all current + future tables
# MAGIC 4. **Business-Driven** → Finance team gets financial data, analysts get anonymized data
# MAGIC 5. **Compliance Ready** → GDPR, HIPAA, PCI-DSS enforcement at the database layer
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ## ⚠️ Important Notes
# MAGIC
# MAGIC * **Row Filter Limitation**: Row filter policies have limited support in some UC environments. For row-level filtering, use WHERE clauses with the `filter_by_region()` UDF in your queries.
# MAGIC * **Kernel Restart Required**: After updating the xref table, you MUST restart the Python kernel for group changes to take effect.
# MAGIC * **Policy Owner Exemption**: As the policy creator, you see all data unmasked by default. This is by design.
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC ## 📝 Next Steps
# MAGIC
# MAGIC ✅ **Demo is ready** → Run Cell 34 to see it in action  
# MAGIC ✅ **Test group switching** → Follow Option 1 above  
# MAGIC ✅ **Add colleagues** → Use Cell 29 or Settings → Groups  
# MAGIC ✅ **Cleanup when done** → Run cells 54-61 in order  
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC **🎬 Ready to demo? Scroll to Cell 34 and run it!**
